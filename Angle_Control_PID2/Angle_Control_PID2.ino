...
void setup()
{
  ...
  Setpoint = desiredCount;
}

void loop() {  
    Input = int(_EncoderTicks);  

    if((millis()-lastMilli) >= LOOPTIME)   {      // Enter loop when millis has reached LOOPTIME 
       lastMilli = millis();  // Save current millis to lastMillis to compare for next loop
